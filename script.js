...
function updateAlivePlayersList(alivePlayers) {
  ...
  const aliveFragment = document.createDocumentFragment();
  const deadFragment = document.createDocumentFragment();
  ...
  if (lobbyPlayers.length > 0) {
    ...
    deadPlayers.forEach(player => {
      ...
      deadFragment.appendChild(li);
    });
  }

  // Append fragments in one operation
  aliveList.appendChild(aliveFragment);
  deadList.appendChild(deadFragment);
}
...
socket.on("day-phase", ({ eliminated, protected, investigationResults, alivePlayers, timer, showNightResults }) => {
  ...
  if (showNightResults) {
    ...
    if (eliminated && protected && eliminated === protected) {
      message += `🛡️ ${eliminated} was attacked but saved by the Queen!<br>`;
    } else if (eliminated && (!protected || eliminated !== protected)) {
